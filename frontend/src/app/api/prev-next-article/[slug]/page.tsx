...
export default async function ArticlePage({ params }: Props) {
  ...
  const navRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/article/navigation/${params.slug}`, { cache: "no-store" })
          ...
          <div className="w-full max-w-[960px]">
            <article className="p-6 text-[0.875rem] leading-[1.6]">
              ...
              <div className="flex justify-between items-center mt-8 border-t pt-4">
                {navigation.previous ? (
                  <Link 
                    href={`/article/${navigation.previous.slug}`}
                    className="flex items-center gap-2 text-[#36c] hover:text-[#447ff5]"
                  >
