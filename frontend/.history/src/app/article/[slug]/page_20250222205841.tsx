...
export default async function ArticlePage({ params }: Props) {
  ...
  const [articleRes, sequenceRes] = await Promise.all([
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/article/${params.slug}`, { cache: "no-store" }),
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/article-sequence/${params.slug}`, { cache: "no-store" })
  ]);

  if (!articleRes.ok) return <div>Article not found</div>
  const article = await articleRes.json()
  
  // Get the sequence data if available
  const sequence = sequenceRes.ok ? await sequenceRes.json() : { previous: null, next: null };
